...
# Function to speak the command (in a separate thread)
def speak_command(command):
    def run_speech():
        engine.say(command)
        engine.runAndWait()
    # Create and start a new thread for each command to avoid blocking
...
while True:
    ...
    for detection in results[0].boxes.data:  # Iterate over detected boxes
        ...
        # Speak the command without blocking the video feed
        speak_command(f"{object_name} detected {position}. {command}.")
